...
{
    public class TheFearedEmpressHandLeft : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (otherHand.active && otherHand.type == ModContent.NPCType<TheFearedEmpressHandRight>())
                {
            ...
            {
                //actual ai
                if (empressAttackType == TheFearedEmpress.EmpressAttacks.BasicFlyMovement || empressAttackType == TheFearedEmpress.EmpressAttacks.ShootIchorRing)
                {
        ...
        public void DoExtraAttacks(int type, Player player, bool phaseTwo)
        {
            ...
            if (type == 2)
            {
                ...
                if (AttackTimer[1] % 30 == 0 && Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    Vector2 dir = Vector2.Normalize((player.Center + player.velocity * 30f ) - NPC.Center);
                ...
                NPC.rotation = NPC.DirectionTo(player.Center + player.velocity * 30f).ToRotation() - (float)Math.PI / 4f;
            }
        }
        ...
        public void DoHandSmashes(int type, Player player, bool phaseTwo, bool halfHealth)
        {
            if (type == 1)
            {
                ...
                if (AttackTimer[0] == 0)
                {
                    ...
                    {
                        float speed = Main.masterMode ? 5f : Main.expertMode ? (phaseTwo ? 4f : 3f) : (phaseTwo ? 3f : 2f);
                        NPC.rotation = NPC.velocity.X * 0.015f;
            ...
            if (type == 1)
            {
                if (AttackTimer[0] == 0f)
                {
                    ...
                    else
                    {
                        ...
                        NPC.velocity = Vector2.Zero.MoveTowards(pos - NPC.Center, 40f);
                    }
                }
                ...
                if (AttackTimer[0] == 1f)
                {
                    ...
                    if (AttackTimer[1] == 1f)
                    {
                        float dashSpeed = Main.masterMode ? 20f : Main.expertMode ? 16f : 12f;
                    ...
                    {
                        NPC.rotation = NPC.velocity.ToRotation() - (float)Math.PI / 4f;
        ...
        public void DoDualHandDashes(NPC empress, Player player, bool phaseTwo)
        {
        ...
        {
            Texture2D IdleTexture = Mod.Assets.Request<Texture2D>("NPCs/Bosses/TheFearedEmpress/TheFearedEmpressHandLeft").Value;
